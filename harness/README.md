...
| `run-ref.ts` | 在固定 QEMU 镜像里运行准备好的 SO3 corpus artifact 两次，校验 serial、run_id、hash、RET 与逐 run 非 idle 计数的确定性 | `results/ref/<profile>/{serial,counts}-N.*`、`summary.json` |
...
bun harness/run-ref.ts --profile virt64-bench --artifacts <dir> --segmap dist/shell/so3-aarch64/segmap.txt
